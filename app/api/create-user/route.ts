...
// Public client (to decode caller's bearer token)
const ANON = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
)

// Service-role client (secure admin ops)
...
export async function POST(req: Request) {
  try {
    // ---- 1) Verify caller is signed in (bearer token) ----
    ...
    // ---- 2) Check caller's role via profiles.role_id -> roles.(name|role|role_name) ----
    ...
    // Normalize roles to single object (some schemas return an array)
    const rAny = Array.isArray((callerProf as any).roles)
      ? (callerProf as any).roles[0]
      : (callerProf as any).roles

    const callerRole = String(rAny?.name ?? rAny?.role ?? rAny?.role_name ?? '')
      .toLowerCase()
      .trim()
    ...
    const { email, password, full_name, role } = (await req.json()) as {
      ...
      role: 'admin' | 'manager' // creating only admin/manager via this endpoint
    }
    ...
    // ---- 5) Resolve role_id to store on profiles (match by name/role/role_name) ----
    const { data: allRoles, error: rolesErr } = await SRV
      .from('roles')
      .select('id, name, role, role_name')

    if (rolesErr || !allRoles) {
      return NextResponse.json({ ok: false, error: 'Roles lookup failed' }, { status: 400 })
    }
    ...
    // ---- 6) Upsert profile with user_id + role_id ----
